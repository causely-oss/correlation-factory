...
export default function ApiDocs() {
  ...
  const endpoints = {
    ...
    metric: {
      ...
      description:
        "Get correlation data for a specific seed and correlation ID. Metrics with the same correlation ID will always correlate, allowing users to pick any two or more metrics and they will have a correlation. The first letter of the seed determines if either the metric is devops (A-M) or absurd (N-Z).",
      parameters: [
        {
          name: "seed",
          type: "string",
          required: true,
          description:
            "Seed for reproducible random generation. The first letter determines the category of the metric (devops (A-M) or absurd (N-Z)).",
        },
        {
          name: "correlationId",
          type: "string",
          required: true,
          description:
            "Unique identifier for the correlation. Metrics with the same correlation ID will always correlate, making it easy to create fake correlations.",
        },
      ],
      ...
      parameters: [
        {
          ...
          type: "string",
          required: true,
          description: "Unique identifier for the correlation",
        },
      ],
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="API documentation for Correlation Factory endpoints"
        />
      </Head>
        ...
        <div className="top-menu">
          ...
          <div className="top-menu-right">
            <div className="top-menu-controls">
              <Link href="/" className="back-link-compact" title="Back to Game">
        ...
        <div className="main-content">
          <div className="api-docs">
            <div className="api-header">
              ...
              <p>
                Interactive documentation for Correlation Factory API endpoints
              </p>
            </div>
                          ...
                          (param, index) => (
                            <div key={index} className="param-item">
                              <div className="param-header">
                                ...
                                <span
                                  className={`param-type ${param.required ? "required" : "optional"}`}
                                >
                                  {param.type}{" "}
                        ...
                        {endpoints[activeEndpoint].parameters.map(
                          (param, index) => (
                            <div key={index} className="form-group">
                              <label
                                htmlFor={`${activeEndpoint}-${param.name}`}
                              >
                                {param.name}{" "}
                                {param.required && (
                              ...
                              <input
                                ...
                                value={
                                  testParams[activeEndpoint][param.name] || ""
                                }
                                onChange={(e) =>
                                  setTestParams((prev) => ({
                                    ...prev,
                      ...
                      {testResults[activeEndpoint] && (
                        <div className="test-results">
                          ...
                          <div className="response-info">
                            <span
                              className={`status ${testResults[activeEndpoint].status === 200 ? "success" : "error"}`}
                            >
                              Status: {testResults[activeEndpoint].status}
                            </span>
                            <span className="url">
                              URL: {testResults[activeEndpoint].url}
                            </span>
                          </div>
                          <pre className="response-data">
                            {JSON.stringify(
                              testResults[activeEndpoint].data,
                              null,
                              2,
                            )}
                          </pre>
